...
def list_contains_round(rounds, number):
    ...
    if number in rounds :
        return True
        """
        if i dont write else :
                            return False 
                            the output is case false will return none
        """
    else :
        return False
...
def approx_average_is_average(hand):
    ...
    true_avg = sum(hand) / len(hand)
    approx1 = (hand[0] + hand[-1]) / 2
    approx2 = hand[len(hand) // 2]

    return true_avg == approx1 or true_avg == approx2


def average_even_is_average_odd(hand):
    """Return if the (average of even indexed card values) == (average of odd ind
3 / 7 Tasks Completed
Code Run
